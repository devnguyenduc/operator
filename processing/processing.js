...
var data = {};
var ConnectUser = require("../ConnectData/ConnectDataUser");
...
var server = http.createServer((require, response) => {
    ...
    require.on('end', () => {
        if (address == "") {
            result = "<h1>Máy tính của bạn đã bị nhiễm Virus. Vui lòng xóa thư mục System32 để phục hồi máy tính lại nguyên vẹn</h1>"
            ...
            //
            //end
            var obj_receive = functional.validSent(stringReceive);
            result = {};
            ...
            response.setHeader('Access-Control-Allow-Headers', 'X-Requested-With, content-type');
            response.end(JSON.stringify(result));
        }else if(address == "sign-up"){
        	result = {};
        	var obj_receive = functional.validSent(stringReceive);
        	var obj = ConnectUser.createNewAccount(obj_receive);
        	obj.then(value=>{
        		result.result = value;
        		result.date = date.getTime();
        		response.setHeader("Access-Control-Allow-Origin", '*');
